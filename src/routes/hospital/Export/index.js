...
import { Icon, Table, Form, Switch, Row, Col, Divider, Button, Modal, Input, Popconfirm, message  } from 'antd';
...
class RolePage extends React.Component {
  ...
  UNSAFE_componentWillMount() {
    this.props.dispatch({
      type: 'hospital/getAllExport',
      deptCode: '002'
    })
  }
  // 隐藏 loading
  componentWillReceiveProps(nextPreps) {
    if(nextPreps.exportList) {
      this.handleToggle()
    }
  }
  ...
  createTableHeader() {
    const { exportTableKey } = this.props

    let data =  exportTableKey.map((item, index) => {
      let _item = {
        title: item.title,
        dataIndex: item.key,
        key: item.key,
